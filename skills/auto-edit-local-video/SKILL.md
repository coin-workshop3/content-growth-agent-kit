...
description: "Build reviewable local video drafts with two explicit FFmpeg modes: talking-head cleanup and scripted material assembly. Use when an agent needs to recommend a mode, inspect local media, optionally run local Whisper transcription, mark conservative filler/repetition candidates, detect long pauses, use a reviewed transcript for sentence-safe talking-head cuts, generate SRT captions, match script segments to tagged product assets, diagnose missing shots, or render a 9:16 draft without uploading media or publishing to a platform."
---
...
- Filler/repetition analysis may mark candidates such as hesitation sounds and adjacent repeated connectors. Keep `automatic_deletion: false`; listen in context before changing `keep/delete`.
...
- Whisper may omit spoken fillers. Candidate timestamps estimated from a segment are not exact word boundaries and must never drive automatic deletion.
